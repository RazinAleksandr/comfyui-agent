...
@/root/workspace/avatar-factory/.claude/get-shit-done/references/ui-brand.md
</required_reading>
...
```bash
INIT=$(node "/root/workspace/avatar-factory/.claude/get-shit-done/bin/gsd-tools.cjs" init plan-phase "$PHASE")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(node "/root/workspace/avatar-factory/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-researcher 2>/dev/null)
AGENT_SKILLS_PLANNER=$(node "/root/workspace/avatar-factory/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-planner 2>/dev/null)
AGENT_SKILLS_CHECKER=$(node "/root/workspace/avatar-factory/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-checker 2>/dev/null)
```
...
```bash
PHASE_INFO=$(node "/root/workspace/avatar-factory/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
node "/root/workspace/avatar-factory/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
DISCUSS_MODE=$(node "/root/workspace/avatar-factory/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.discuss_mode 2>/dev/null || echo "discuss")
```
...
```bash
PHASE_DESC=$(node "/root/workspace/avatar-factory/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick section)
```
...
**If found:**
1. Read template: `/root/workspace/avatar-factory/.claude/get-shit-done/templates/VALIDATION.md`
...
```bash
UI_PHASE_CFG=$(node "/root/workspace/avatar-factory/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(node "/root/workspace/avatar-factory/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(node "/root/workspace/avatar-factory/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" 2>/dev/null)
...
2. Disable Nyquist with the exact command:
   `node "/root/workspace/avatar-factory/.claude/get-shit-done/bin/gsd-tools.cjs" config-set workflow.nyquist_validation false`
