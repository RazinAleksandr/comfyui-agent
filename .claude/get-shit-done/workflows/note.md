...
- **Global scope**: `/root/workspace/avatar-factory/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/root/workspace/avatar-factory/.claude/notes/`)
...
2. Glob `/root/workspace/avatar-factory/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/root/workspace/avatar-factory/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/root/workspace/avatar-factory/.claude/notes/` — works in any directory
